 ...
 */
class WeChatMessageIndx extends BaseContainer {
    searchData: any;
    //初始化加载
    constructor(props) {
        /*
    ...
    messageNoticeOnClick(Id, record) {
        ...
        TemplateApi.getMessageTemplateList({ "Id": Id, "MessageTemplateType": 2 }).then(function (data) {
            if (data.IsOK) {
                var functionData = [];
                if (Array.isArray(data.Value)) {
                    functionData = data.Value;
            ...
            TemplateApi.updateMessageTemplateList(obj).then(function (data) {
                if (data.IsOK) {
                    Modal.success({
                        ...
                        content: '消息通知模板保存成功',
                        onOk() {
                            Tool.goPush('Manager/MessageTemplate/WeChat');
                        },
                    });
                } else {
    ...
    render() {
        ...
        var self = this;
        var getTemplateParameter = (TemplateParameter, record) => {
            var parameterHtml = [];
            for (var item in TemplateParameter) {
                parameterHtml.push(<span key={"span_" + self.state.selectedRowId + item}><span>#{item}#</span>
                    <Input type="text" className="cp1 form-control margin-top10 margin-btm30" data-key={item}  defaultValue={TemplateParameter[item]} onChange={ (e) => { self.inputonChange(e, record) } } /></span>);
            }
        ...
        return (
            <AppBody>
                <div className="main-content-title padding-top15 clearfix">
                    <a className="main-content-word pull-left" href="/Manager/MessageTemplate/Index">短信模板设置</a>
                    <a className="main-content-word pull-left set-content-word-te" href="/Manager/MessageTemplate/WeChat">微信模板设置</a>
                </div>
                <div className="col-xs-12 margin-left20 margin-top30">
                    <ul className="message-kuang pull-left">
                        <a href="javascript:;"><li className="message-kuang-te">需发微信点</li></a>
                                ...
                                return (<div key={"div_" + index}>
                                    ...
                                    <div className="message-Nei pull-left">
                                        ...
                                        <div className="message-Nei2">
                                            <p>模板内容</p>
                                            <Input type="textarea" rows={5} className="cp1 form-control" disabled value={item.TemplateContent} />
...
let WeChatMessageIndxPage = Form.create({})(WeChatMessageIndx);

/**
 * 添加监听数据
 */
const App = connect(mapStateToProps)(WeChatMessageIndxPage);
