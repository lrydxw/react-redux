...
import { Table, Icon, Row, Col, Modal, Form, Input, Upload, Tabs, Radio, Switch, FormComponentProps, message, Button } from 'antd';
...
interface ShopHomeBannerConfigFormProps extends FormComponentProps {
...
class ShopHomeBannerConfigForm extends React.Component<ShopHomeBannerConfigFormProps, any> {
    ...
    //插入真实DOM之前被执行 
    componentWillMount() {
        console.log('执行组件Banner componentWillMount');
    }


    //插入真实DOM之后被执行
    componentDidMount() {
        this.initShopHomeBannerList();
        console.log('执行组件 Banner componentDidMount');
    ...
    //更新DOM之前被执行
    componentWillUpdate() {
        console.log('执行组件 Banner componentWillUpdate');
    }

    //更新DOM之后被执行
    componentDidUpdate(nextProps) {

        console.log('执行组件 Banner componentDidUpdate');
    }
    //移除DOM之前被执行
    componentWillUnmount() {
        console.log('执行组件 Banner componentWillUnmount');
    }
    //已加载组件收到新的参数时调用
    componentWillReceiveProps(nextProps) {
        console.log('执行组件 Banner componentWillReceiveProps');
    ...
    submitForm() {
        ...
        form.validateFields((errors, values) => {
            ...
            var shopBannerConfig = [];
            _this.state.shopBannerList.map(function (item, i) {
                var shopBanner = new Object();
                shopBanner["Id"] = item.Id;
                shopBanner["Image"] = obj["Image_" + item.Id];
                shopBanner["Link"] = obj["Link_" + item.Id];
                shopBanner["SortIndex"] = item.SortIndex;
                shopBanner["Title"] = item.Title;
                shopBanner["EditType"] = item.EditType;
                shopBannerConfig.push(shopBanner);
            });

            console.log(obj);
            console.log(shopBannerConfig);

            ShopHomeSetApi.updateShopBanner({ ShopBannerList: shopBannerConfig }).then(function (data) {
                if (data.IsOK) {
                    Modal.success({
                        title: '保存成功',
                        content: 'Banner图配置已保存',
                    });
...
let ShopHomeBannerConfigFormContent = Form.create()(ShopHomeBannerConfigForm)

export default ShopHomeBannerConfigFormContent;