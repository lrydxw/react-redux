 ...
 */
class PromotionTemplateAdd extends BaseContainer {
    constructor(props) {
        /*
            1、初始化时，将被执行
    ...
    GetPromotionTemplateInfo() {
        var _this = this;
        var DataObj = DataObj || {};
        DataObj.Id = _this.state.promotionTemplateId;
        DataObj.TemplateName = "";//模板名称
        DataObj.TemplateImagePath = ""; //模板底图
        DataObj.TemplateDesc = ""; //模板描述
        DataObj.ExampleImage = ""; //模板样例图               
        DataObj.IsEnabled = false;    //模板是否启用     
        DataObj.TemplateConfigs = []; //配置项数组 

        //对页面元素进行赋值
        _this.bindValue(DataObj);
    ...
    GetAllTemplateConfigEnum() {
        ...
        PromotionTemplateAPI.GetAllTemplateConfigEnum(obj).then(function (data) {
            if (data.IsOK) {
                _this.setState({ ConfigsEnum: data.Value });
            }
        });
    }
    ...
    handelSubmit(isPreview) {
        ...
        _this.props.form.validateFields((errors, values) => {
            ...
            var result = {
                "Id": obj.Id,
                "TemplateType": _this.state.promotionTemplateType,
            ...
            PromotionTemplateAPI.AddPromotionTemplate(result).then(function (data) {
                if (data.IsOK) {
                    Modal.success({
                        title: '操作成功',
                        content: "",
    ...
    addNewConfig(typeName, typeValue) {
        ...
        var DataObj = DataObj || {};
        DataObj.Id = _this.state.promotionTemplateId;
        DataObj.TemplateName = getFieldValue("TemplateName");//模板名称
        DataObj.TemplateImagePath = getFieldValue("TemplateImagePath"); //模板底图
        DataObj.TemplateDesc = getFieldValue("TemplateDesc"); //模板描述
        DataObj.ExampleImage = ""; //模板样例图               
        DataObj.IsEnabled = getFieldValue("IsEnabled");    //模板是否启用     
        ...
        DataObj.TemplateConfigs = _this.state.promotionConfigs; //配置项数组 
        _this.bindValue(DataObj);
    }
        ...
        return (
            <AppBody>
                ...
                <div className="main-content-title padding-top35 clearfix">
                    <a className="main-content-word pull-left  set-content-word-te">{_self.state.promotionTemplateType}</a>
                </div>

                <div className=" editor-left8 ">
                    <Affix >
                        <div className=" padding-top20 padding-btm20 pull-left ">
                            <div className=" editor-left" style={{ height: "auto", display: getFieldValue("TemplateImagePath") == "" ? "none" : "" }}>
                                <div>
                                    <FormItem>
...
let editPage = Form.create({})(PromotionTemplateAdd);
