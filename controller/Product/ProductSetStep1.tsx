 ...
 */
class ProductSetStep1 extends BaseContainer {
    constructor(props) {
        /*
            1、初始化时，将被执行
        ...
        ProductApi.getProductBasicInfo(obj).then(function (data) {
            if (data.IsOK) {
                ...
                if (dataObj.SelectGiftDataList.length > 0) {
                    var currentGiftList = dataObj.SelectGiftDataList;
                    var giftDataList = _this.state.giftData;
                    for (var i = 0; i < currentGiftList.length; i++) {
                        var giftName = "";
                        var index = _this.arrayIndex(giftDataList, currentGiftList[i]);
                        if (index > -1) {
                            currentGiftList[i].GoodsName = giftDataList[index].GoodsName;
                        }
                ...
                _this.state.siteName = dataObj.SiteName;
    ...
    */
    ...
    submitProductInfoSetStep1(step) {
        ...
        this.props.form.validateFields((errors, values) => {
            ...
            if (obj.Id.length <= 0) {
                ...
                ProductApi.insertProductBasicInfo(obj).then(function (data) {
                    if (data.IsOK) {
                        if (step == 2) {
                            _this.nextStep2(data.Value);
                        }
                        else if (step == 3) {
                            _this.nextStep3(data.Value);
                        }
                ...
                ProductApi.updateProductBasicInfo(obj).then(function (data) {
                    if (data.IsOK) {
                        if (step == 2) {
                            _this.nextStep2(obj.Id);
                        }
                        else if (step == 3) {
                            _this.nextStep3(obj.Id);
                        }
                    } else {
                        message.error(data.Message);
                    }
    ...
    loadCommunityData() {
        var _this = this;
        CommunityApi.getCommunitySelectData({ ProductType: 1 }).then(function (data) {
            if (data.IsOK) {
    ...
    render() {
        ...
        const introductionProps = getFieldProps('Introduction', {
            validate: [{
                rules: [
                    { required: false, message: '请输入商品简介' },
                ], trigger: ['onBlur', 'onChange'],
            }]
        });
                                    ...
                                    <div className="form-horizontal tasi-form" >
                                        <div className="form-group">
                                            <div className="row margin0">
                                                <label className="control-label col-xs-2"><span className="color-red">*</span>商品标题：</label>
                                            ...
                                            <div className="row margin0">
                                                ...
                                                <div className="col-xs-5">
                                                    <FormItem
                                                        ...
                                                        >
                                                        <Input type="hidden" className="cp1 form-control" placeholder="商品图" {...showImgProps} />
                                                        <div>
                                                            <Upload {...showImgUploadProps} name="upload" >
                                                                <Icon type="plus" />
                                                                <div className="ant-upload-text" >商品图</div>
                                                            </Upload>
                                        ...
                                        <div className="form-group">
                                            <div className="row margin0">
                                                <label className="control-label col-xs-2">商品简介：</label>
                                                <div className="col-xs-5">
                                                    <FormItem key="Introduction"

                                                        >
                                                        <Input rows="5" className="cp1 form-control" type="textarea" {...introductionProps} placeholder="请输入商品简介" />
                                                    </FormItem>
                                                </div>
                                            </div>
                                        </div>
...
let ProductSetStep1Page = Form.create({})(ProductSetStep1);
export { ProductSetStep1Page }
